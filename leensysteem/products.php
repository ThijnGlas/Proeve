...
<div class="middleSection">
    <div class="productCard-container">
        ...
        if (isset($_POST['zoekenInput']) && trim($_POST['zoekenInput']) != "") {
            ...
            $productsFromDatabase = mysqli_query($connection, "SELECT * FROM products WHERE (name LIKE '%" . $_POST['zoekenInput'] . "%' OR id = '" . $_POST['zoekenInput'] . "' OR model_type LIKE '%" . $_POST['zoekenInput'] . "%') AND amount > 0") or die(mysqli_error($connection));
            if (mysqli_num_rows($productsFromDatabase) == 0) {
                header("location: ?page=products&notfound=1");
                $productsFromDatabase = mysqli_query($connection, "SELECT * FROM products WHERE amount > 0 ORDER BY id DESC");
            }
        } 
        ...
        elseif(array_key_exists('category', $_GET)){
            ...
            if ($categoryId == 1){
                ...
                $productsFromDatabase = mysqli_query($connection, "SELECT * FROM products WHERE category = $categoryId AND amount > 0 ORDER BY id DESC");
